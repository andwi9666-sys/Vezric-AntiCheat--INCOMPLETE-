...
public class CombatAnalyzerAccuracySpikeTest {
    ...
    @Test
    public void analyzeHitIncludesAccuracySpikeResult() {
        ...
        Assert.assertTrue(result.getReasons().stream().anyMatch(
                reason -> reason.contains("Accuracy spike") || reason.contains("Strong accuracy spike")));
    }

    @Test
    public void analyzeHitAddsSpikeScoreOnTopOfGeometryAndPreAim() {
        List<CombatSample.RotationPoint> rotations = new ArrayList<CombatSample.RotationPoint>();
        for (int i = 0; i < 5; i++) {
            rotations.add(new CombatSample.RotationPoint(0.0F, 0.0F, 100L + i));
        }
    ...
    @Test
    public void analyzeAccuracySpikeDelegatesToAnalyzer() {
        List<CombatSample.RotationPoint> rotations = new ArrayList<CombatSample.RotationPoint>();
        for (int i = 0; i < 5; i++) {
        ...
        CombatSample sample = facingTargetSample(2.5D, -90.0F, 0.0F, rotations);
        CombatHitResult current = CombatHitClassifier.classify(sample, analyzer.getConfig());
        Assert.assertEquals(0.0D, analyzer.analyzeAccuracySpike(sample, current).getSuspiciousScore(), 0.001D);
    }
