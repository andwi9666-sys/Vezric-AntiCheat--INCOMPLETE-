...
public final class PrismPacketOrderA extends TierCheck {

    public PrismPacketOrderA(VezAntiCheat plugin) {
        super(plugin, "PrismPacketOrderA", CheckTier.PRISM);
    }
    ...
    private void evaluate(Player p, PlayerData data, long nowMs) {
        ...
        if (incrementBuffer(p, suspicion >= threshold * 1.35D ? 2 : 1)) {
            fail(p, data, plugin.tierCfg().checkDouble(name(), "failVl", 1.0D),
                    "order suspicion=" + round3(suspicion) + " kind=ATTACK_WITHOUT_MOVE");
            resetBuffer(p);
        }
    }
}
